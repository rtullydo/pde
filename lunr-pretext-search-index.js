...
},
{
  ...
  "body": " Generalized functions and functions   Linear algebraic motivation  The basic idea of the theory of Green's functions is to construct complete solutions out of superpositions of fundmental solutions. To get the thematic idea at play, consider the linear algebraic equation In this example, imagine that the system describes a system of masses interconnected by springs (which is recorded by the matrix ) where the th entry of is the displacement of the th mass from equilibrium, and the entries of describe the force applied to the th mass.  Let denote the th elementary basis vector. We can interpret as representing a unit impulse concentrated at mass . Then applying that force to the system gives so that , which is the vector that records the response of the system to . The key observation is that any force vector can be decomposed into a superposition of impulses and the corresponding response to can be written as a superposition of responses to impulses . This is the same gameplan we'll pursue in the context of linear differential equations - find the response to concentrated unit impulses, and then superimpose those responses to get a general solution.    Delta functions  Suppose that we're trying to solve a linear boundary value problem on . The unit impulse at a point is given by the delta function , .   In order to concentrate force at a single point, we require that     In order to ensure that the total force is of magnitude 1, we require that    Of course, these are inconsistent conditions - a function that is identically 0 except at a single point must have integral 0 in the standard Riemann theory. So must be something else, something not a function ! Instead, belongs to a broader family of mathematical objects called distributions or generalized functions . In fact, the delta function would more properly be called the delta distribution. However, tradition and usage lead us to continue with the usual naming convention.  There are two typical intuituve ways to understand the function. Both have value, but neither is precise. The first is almost certainly more familiar.   The following discussion is a heuristic, not a rigorous treatment - however, in taking this approach we are following in the footsteps of Cauchy, Green, and Heaviside, who developed and used this machinery well before it was put on firm mathematical footing. Heaviside once said Shall I refuse my dinner because I do not understand the process of digestion? We too shall eat.   The first method is to construct as a limit of appropriately chosen functions. We've already seen such a method in the context of the heat kernel. Here is another family of functions that leads to the delta function. Let be a family of functions on . Then the pointwise limit of is and   provisional definition of   The delta function is given by The delta function at is given by     The previous definition isn't quite rigorous in some fundamental ways. For example, we still have the issue that Moreover, defined this way has no meaningful value at .  The second approach we'll take is closer to the real story. We'll leverage some ideas from inner product spaces (the study of spaces of functions by way of linear algebra is called functional analysis ). Let be a continuous function on . Assume that makes sense. Then Define a map by Because is defined by an integral, it is a linear map, in the sense that A linear map acting on a vector space and mapping into scalars is called a linear functional . In fact, every function defines a linear functional via . We can record this information using the inner product notation . Restating our previous discussion in these terms, every function induces a linear functional by   In finite dimensional vector spaces, the converse is true - that is, every linear functional has a representing vector such that This is a much trickier statement in infinite dimensional settings, and yet we can proceed formally. Certainly, the evaluation map is a linear functional since If was to have a representing vector so that then our previous discussion about the delta function would give that satifies the requirement. The existence of such a vector is a non-trival theorem, and requires that we work in the dual space of , denoted , which is the space of linear functionals acting on . We'll spend more time talking about dual spaces when we introduce the notions of weak derivatives and weak convergence in the next section. For now, let us continue our discussion of the delta function.    Properties of the delta function  We will interchange between both methods of understanding as befits our current exploration. First, then, let us note that we can form linear combinations of delta functions. Let . Then   Second, we can integrate . One approach to discovering the formula is to integrate and take a limit. Indeed, Let us choose (this is a compatibility condition with Fourier series). Then That is, the integral of is the unit step function . Then, motivated by the FTC for functions, we declare that which gives a method that allows differentiation to extend to discontinuous functions.  The derivative above is actually an instance of the distributional derivative . Let us become more familiar with this type of differentiation by looking at the derivative of itself. Assume that . First, we use the identification If we could swap out the for a , we'd be in business. Fortunately, this is precisely what integration by parts will allow us to do. Formally, Hence we define to be the distribution so that for all appropriate , we have    We're going to play slightly loose with the distinction between weak and distributional ideas in this discussion. If you're interested in digging deeper on the distinction, you should consult a graduate level analysis text like Folland's Modern Analysis .   When we write , we are referring to a weak derivative, as is not a function, and hence does not have a functional derivative.    Suppose that . A function is called a weak derivative of if for all functions with .     Generally, we use the adjective weak to describe behavior that happens in an inner product. Earlier, we claimed that we could take to understand the delta function as This limit makes no sense as it does not converge at all at and the limiting object isn't even a function. What we really meant was that that is, the linear functionals represented by converge to the linear functional of evaluation at . This is an example of what we call weak convergence .    A sequence converges weakly to if In this case we write .    As another example, consider the the vector space and the sequence . This sequence has no pointwise limit at all, nor does it converge in . However, , Hence, . That the limit of the integral above tending to 0 is a consequence of the Riemann-Lebesgue Lemma .  Now equipped with a notion of convergence appropriate to working with the delta function, let us try to produce a Fourier series for . Let us consider . The Fourier series representation for should take the form where and Hence, Since isn't a function, the Fourier series can't converge to pointwise or in . However, if we denote , then Hence .    A quick introduction to distributions  So what is really? First, given a linear functional , we test the functional with inputs to see its structure. The family of test functions is the space . These are functions that are infinitely differentiable on and have compact support , which means that all of the non-zero values of such a function are contained in a closed interval. In particular, this means that if then is actually 0 not just at but in a small interval extending from and .  The space of distributions is the space of continuous linear functionals on .    The delta function is the distribution on so that      "
},
{
  "id": "subsec-delta-functions-4",
  "level": "2",
  "url": "sec-generalized-functions-and-m-delta-m-functions.html#subsec-delta-functions-4",
  "type": "Warning",
  "number": "3.1.1",
  "title": "",
  "body": " The following discussion is a heuristic, not a rigorous treatment - however, in taking this approach we are following in the footsteps of Cauchy, Green, and Heaviside, who developed and used this machinery well before it was put on firm mathematical footing. Heaviside once said Shall I refuse my dinner because I do not understand the process of digestion? We too shall eat.  "
},
{
  "id": "def-delta-prelim",
  "level": "2",
  "url": "sec-generalized-functions-and-m-delta-m-functions.html#def-delta-prelim",
  "type": "Definition",
  "number": "3.1.2",
  "title": "provisional definition of <span class=\"process-math\">\\(\\delta\\)<\/span>.",
  "body": "provisional definition of   The delta function is given by The delta function at is given by    "
},
{
  "id": "subsec-properties-of-the-delta-function-5",
  "level": "2",
  "url": "sec-generalized-functions-and-m-delta-m-functions.html#subsec-properties-of-the-delta-function-5",
  "type": "Warning",
  "number": "3.1.3",
  "title": "",
  "body": " We're going to play slightly loose with the distinction between weak and distributional ideas in this discussion. If you're interested in digging deeper on the distinction, you should consult a graduate level analysis text like Folland's Modern Analysis .  "
},
{
  "id": "def-weak-derivative",
  "level": "2",
  "url": "sec-generalized-functions-and-m-delta-m-functions.html#def-weak-derivative",
  "type": "Definition",
  "number": "3.1.4",
  "title": "",
  "body": "  Suppose that . A function is called a weak derivative of if for all functions with .    "
},
{
  "id": "def-weak-convergence",
  "level": "2",
  "url": "sec-generalized-functions-and-m-delta-m-functions.html#def-weak-convergence",
  "type": "Definition",
  "number": "3.1.5",
  "title": "",
  "body": "  A sequence converges weakly to if In this case we write .   "
},
{
  "id": "def-delta",
  "level": "2",
  ...
  "number": "3.1.6",
  "title": "",
  "body": "  The delta function is the distribution on so that    "
},
{
  "id": "backmatter-2",
  "level": "1",
